...
public partial class TemplateNode {
	...
	public sealed class JavaScript(TemplateElementCollection children) : RecursiveTemplateTag(children) {
		public override string Evaluate(RequestProcess process) => new TemplateElementCollection(new Srai(new TemplateElementCollection("JSFAILED"))).Evaluate(process);

		public static JavaScript FromXml(XmlNode node, AimlLoader loader) => new(TemplateElementCollection.FromXml(node, loader));
	}
}
