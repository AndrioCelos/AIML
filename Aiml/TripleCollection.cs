...
namespace Aiml {
    public class TripleCollection {
		private readonly Dictionary<int, Triple> triples = new Dictionary<int, Triple>();  // This is a hashtable and not an array, to support easy removal of arbitrary triples.
		private int lastIndex = -1;
		private readonly Dictionary<string, HashSet<int>> triplesBySubject = new Dictionary<string, HashSet<int>>(StringComparer.CurrentCultureIgnoreCase);
		private readonly Dictionary<string, HashSet<int>> triplesByPredicate = new Dictionary<string, HashSet<int>>(StringComparer.CurrentCultureIgnoreCase);
		private readonly Dictionary<string, HashSet<int>> triplesByObject = new Dictionary<string, HashSet<int>>(StringComparer.CurrentCultureIgnoreCase);
