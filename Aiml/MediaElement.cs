...
public class MediaElement(XmlElement element) : IMediaElement {
	...
	internal static List<IMediaElement> ParseInlineElements(Bot bot, XmlElement element) {
			...
			switch (node.NodeType) {
				...
				default:
					if (node is XmlElement childElement && !childElement.Name.Equals("oob", StringComparison.OrdinalIgnoreCase)) {
						if (bot.MediaElements.TryGetValue(childElement.Name, out var data)) {
							...
							elements.Add(data.reviver(bot, childElement));
						} else {
